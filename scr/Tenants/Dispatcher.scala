...
// This will schedule to tick after 0ms repeating every interval
 val cancellable = system.scheduler.schedule(new FiniteDuration(0, TimeUnit.SECONDS), 
     new FiniteDuration(TenantManagerObject.getInstance().getInterval(), TenantManagerObject.getInstance().getIntervalUnit()))
     {
        println("CyberorgManager-Tick")
        // reset the time for all cyberorgs
        TenantManagerObject.getInstance().resetExecutionTime()
  ...
  protected[akka] override def registerForExecution(mbox: Mailbox, hasMessageHint: Boolean, hasSystemMessageHint: Boolean): Boolean = {  
    ...
    if((mbox.actor ne null) && !TenantManagerObject.getInstance().isSchedullable(mbox.actor.self))
    {
      // store the mailbox
